import { authMiddleware } from '@apex/auth/proxy';
import { internationalizationMiddleware } from '@apex/internationalization/proxy';
...
export default authMiddleware(async (_auth, request, event) => {
  ...
  const middlewareResponse = await composedMiddleware(request as unknown as NextRequest, event);
