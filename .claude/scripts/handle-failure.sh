...
if echo "$COMMAND" | grep -qE '(npm run build|next build)' && echo "$ERROR" | grep -qiE '(build.*fail|module not found|cannot find)'; then
  SUGGESTIONS="APEX DEBUG: Build failure. Check: missing imports, environment variables, or server-only code in client components. Read the FULL error — the first error is usually the root cause, the rest are cascading."
fi
...
if [ "$RECENT_FAILURES" -ge 3 ]; then
  echo ""
  echo "⚠️ APEX DEBUG: ${RECENT_FAILURES} failures in the last 60 seconds. STOP and rethink."
  echo "You may be in a retry loop. Step back and:"
  echo "  1. Read the FULL error from the first failure"
  echo "  2. Understand the root cause before trying again"
  echo "  3. Check if the approach itself is wrong, not just the execution"
