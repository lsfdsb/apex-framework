...
describe('PageShell', () => {
  ...
  it('renders optional sidebar when provided', () => {
    render(<PageShell sidebar={<nav data-testid="sidebar">Sidebar</nav>}>content</PageShell>);
    expect(screen.getByTestId('sidebar')).toBeInTheDocument();
  });

  it('renders optional header when provided', () => {
    render(
      <PageShell header={<header data-testid="app-header">Header</header>}>content</PageShell>,
    );
  ...
  it('renders optional mobileNav when provided', () => {
    render(
      <PageShell mobileNav={<div data-testid="mobile-nav">Mobile Nav</div>}>content</PageShell>,
    );
  ...
  it('uses default sidebarWidth of 56 when sidebar provided without explicit width', () => {
    render(<PageShell sidebar={<nav>Sidebar</nav>}>content</PageShell>);
    const main = screen.getByRole('main');
