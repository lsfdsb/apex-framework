    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 6, maxWidth: 640, margin: "0 auto" }}>
      {items.map((t) => (
        ...
        >
          ...
          {/* Info */}
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 2, textDecoration: t.done ? "line-through" : "none", color: t.done ? "var(--text-muted)" : "var(--text)" }}>{t.title}</div>
          ...
          {/* Assignee */}
          <div style={{ width: 20, height: 20, borderRadius: "50%", background: "var(--bg-surface)", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, fontWeight: 600, color: "var(--text-muted)", flexShrink: 0 }}>{t.assignee.initials}</div>

          {/* Due badge */}
          <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 999, fontWeight: 500, whiteSpace: "nowrap", background: dueBg[t.dueType], color: dueColor[t.dueType] }}>{t.due}</span>
        </div>
      ))}
    </div>
  );
