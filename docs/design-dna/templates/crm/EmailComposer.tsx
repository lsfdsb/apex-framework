...
const CHANNELS: { id: Channel; label: string; icon: string; color: string }[] = [
  { id: "phone", label: "Telefone", icon: "M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z", color: "var(--success)" },
  { id: "whatsapp", label: "WhatsApp", icon: "M12 2C6.477 2 2 6.477 2 12c0 1.89.525 3.66 1.438 5.168L2 22l4.832-1.438A9.955 9.955 0 0012 22c5.523 0 10-4.477 10-10S17.523 2 12 2z", color: "#25D366" },
  { id: "email", label: "E-mail", icon: "M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2zM22 6l-10 7L2 6", color: "var(--accent)" },
];

function ToolBtn({ d }: { d: string }) {
  return (
    <button style={{ background: "none", border: "none", color: "var(--text-muted)", cursor: "pointer", padding: 6, borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", transition: "color .2s" }}>
    ...
    <div style={{ background: "var(--bg-elevated)", border: "1px solid var(--border)", borderRadius: "var(--radius, 12px)", overflow: "hidden", maxWidth: 640, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", borderBottom: "1px solid var(--border)" }}>
        {CHANNELS.map((ch) => (
          <button key={ch.id} onClick={() => setChannel(ch.id)} style={{
            ...
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={channel === ch.id ? ch.color : "currentColor"} strokeWidth="1.5"><path d={ch.icon} /></svg>
            {ch.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: "12px 20px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 8, height: 8, borderRadius: "50%", background: CHANNELS.find((c) => c.id === channel)?.color }} />
          <span style={{ fontSize: 13, fontWeight: 500 }}>
            {channel === "phone" ? "Ligação para Elena Rodriguez" : channel === "whatsapp" ? "Elena Rodriguez · +55 11 9xxxx" : "elena@seriesb.com"}
          </span>
        </div>
        {channel === "email" && (
          <div style={{ display: "flex", gap: 8 }}>
            {["Rascunho", "Templates"].map((l) => (
              <button key={l} style={{ fontSize: 11, padding: "4px 10px", borderRadius: "var(--radius-sm, 8px)", border: "1px solid var(--border)", background: "none", color: "var(--text-secondary)", cursor: "pointer", fontFamily: "var(--font-body)" }}>{l}</button>
            ))}
          </div>
        )}
      </div>
      ...
      {channel === "phone" && (
        <div style={{ padding: 32, textAlign: "center" }}>
          <div style={{ width: 72, height: 72, borderRadius: "50%", background: "rgba(52,211,153,0.1)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--success)" strokeWidth="2"><path d={CHANNELS[0].icon} /></svg>
          </div>
          <div style={{ fontSize: 18, fontWeight: 600, marginBottom: 4 }}>Elena Rodriguez</div>
          <div style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 24 }}>+55 11 98765-4321 · StyleHouse</div>
          <div style={{ display: "flex", justifyContent: "center", gap: 12 }}>
            <button style={{ padding: "10px 24px", borderRadius: "var(--radius-sm, 8px)", background: "var(--success)", color: "white", border: "none", fontSize: 14, fontWeight: 500, cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>Ligar agora</button>
          ...
          <div style={{ marginTop: 24, padding: 16, borderRadius: "var(--radius-sm, 8px)", background: "var(--bg-surface)", border: "1px solid var(--border)", fontSize: 12, color: "var(--text-muted)", textAlign: "left" }}>
            <div style={{ fontWeight: 500, color: "var(--text-secondary)", marginBottom: 8 }}>Última ligação · 3 dias atrás</div>
      ...
      {channel === "whatsapp" && (
        <div>
          <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 12, maxHeight: 280, overflowY: "auto" }}>
            ...
            <div style={{ maxWidth: "80%", padding: "10px 14px", borderRadius: 12, fontSize: 14, lineHeight: 1.5, background: "#25D366", color: "white", alignSelf: "flex-end", borderBottomRightRadius: 4 }}>Que bom, Elena! Ajustei o escopo como combinamos. Quer agendar uma call?</div>
          ...
          <div style={{ padding: "12px 20px", borderTop: "1px solid var(--border)", display: "flex", gap: 8 }}>
            ...
            <button style={{ background: "#25D366", border: "none", width: 32, height: 32, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "white" }}>
