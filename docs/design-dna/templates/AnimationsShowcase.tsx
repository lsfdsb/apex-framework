...
import { DnaBackground, type DnaPattern, type DnaAnimatedBg } from "../starters/patterns/DnaBackground";
...
function SectionLabel({ label, heading, description }: { label: string; heading: string; description: string }) {
  return (
    <div className="reveal" style={{ marginBottom: 40 }}>
      <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--accent)", fontWeight: 600, marginBottom: 12 }}>{label}</div>
      <h2 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(32px, 5vw, 56px)", fontWeight: 400, letterSpacing: "-0.03em", lineHeight: 1.1, marginBottom: 12 }}>{heading}</h2>
      <p style={{ fontSize: 16, color: "var(--text-secondary)", fontWeight: 300, maxWidth: 560 }}>{description}</p>
    </div>
  );
}
...
function PathBadge({ path }: { path: string }) {
  return (
    <span style={{ fontFamily: "var(--font-mono, monospace)", fontSize: 11, color: "var(--accent)", background: "var(--bg-surface, var(--bg-elevated))", padding: "4px 8px", borderRadius: 6, display: "inline-block", marginTop: 12, border: "1px solid var(--border)" }}>
      {path}
    </span>
  );
}
...
function AnimDemoCard({ animClass, label, index }: { animClass: string; label: string; index: number }) {
  const [key, setKey] = useState(0);
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, padding: "24px 16px", background: "var(--bg-elevated)", border: "1px solid var(--border)", borderRadius: "var(--radius, 12px)" }}>
      <div key={key} className={animClass} style={{ width: 56, height: 56, borderRadius: "var(--radius-sm, 8px)", background: "var(--accent-glow, rgba(99,107,240,0.12))", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontSize: 22 }}>✦</span>
      </div>
      <code style={{ fontSize: 12, color: "var(--text-secondary)", fontFamily: "var(--font-mono, monospace)" }}>.{animClass}</code>
      <button
        onClick={() => setKey((k) => k + 1)}
        style={{ fontSize: 12, fontWeight: 500, color: "var(--text-muted, var(--text-secondary))", background: "none", border: "1px solid var(--border)", borderRadius: 6, padding: "4px 12px", cursor: "pointer", transition: "all .2s" }}
      >
        Replay
      </button>
      <span style={{ fontSize: 11, color: "var(--text-muted, var(--text-secondary))" }}>{label}</span>
    </div>
  );
}
...
export default function AnimationsShowcase() {
  ...
  // Background picker
  const PATTERN_OPTIONS: DnaPattern[] = ["dots", "grid", "topo", "circuit", "hexagons", "constellation", "waves", "dna"];
  const ANIMATED_OPTIONS: DnaAnimatedBg[] = ["orbs", "aurora", "particles", "gradient", "nebula", "spotlight"];
    ...
    <div style={{ color: "var(--text)", fontFamily: "var(--font-body)" }}>
      ...
      <section style={{ padding: "140px 32px 100px", textAlign: "center" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div className="reveal" style={{ fontSize: 12, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--accent)", fontWeight: 500, marginBottom: 16 }}>
            v5.20 Primitives
          </div>
          ...
          <p className="reveal reveal-delay-2" style={{ fontSize: 20, color: "var(--text-secondary)", fontWeight: 300, maxWidth: 480, margin: "20px auto 0", lineHeight: 1.6 }}>
      ...
      <section style={{ padding: "100px 32px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          />
          <div className="anim-grid-2 reveal" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
      ...
      <section style={{ padding: "100px 32px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          {/* Controls */}
          <div className="reveal" style={{ display: "flex", gap: 16, marginBottom: 24, flexWrap: "wrap" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <label style={{ fontSize: 12, color: "var(--text-secondary)", fontWeight: 500 }}>Pattern</label>
              <select
                value={selectedPattern}
                onChange={(e) => { setSelectedPattern(e.target.value as DnaPattern); setBgKey((k) => k + 1); }}
                style={{ padding: "8px 12px", background: "var(--bg-elevated)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm, 8px)", color: "var(--text)", fontSize: 14, cursor: "pointer", minWidth: 160 }}
              >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <label style={{ fontSize: 12, color: "var(--text-secondary)", fontWeight: 500 }}>Animated layer</label>
              <select
                value={selectedAnimated}
                onChange={(e) => { setSelectedAnimated(e.target.value as DnaAnimatedBg); setBgKey((k) => k + 1); }}
                style={{ padding: "8px 12px", background: "var(--bg-elevated)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm, 8px)", color: "var(--text)", fontSize: 14, cursor: "pointer", minWidth: 160 }}
              >
          ...
          <div
            className="reveal"
            style={{ position: "relative", height: 400, borderRadius: "var(--radius, 12px)", border: "1px solid var(--border)", overflow: "hidden", background: "var(--bg-elevated)" }}
          >
            ...
            <div style={{ position: "absolute", inset: 0, zIndex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8 }}>
              <span style={{ fontFamily: "var(--font-display)", fontSize: 32, fontWeight: 400, letterSpacing: "-0.03em" }}>
                {selectedPattern} + {selectedAnimated}
              </span>
              <code style={{ fontSize: 13, color: "var(--text-secondary)", fontFamily: "var(--font-mono, monospace)" }}>
