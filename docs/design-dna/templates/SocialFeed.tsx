...
function PostCard({ post }: { post: Post }) {
  ...
  return (
    <article className="sf-post">
      ...
      <p
        style={{ fontSize: 15, lineHeight: 1.7, color: 'var(--text-secondary)', marginBottom: 16 }}
      >
        {post.content}
      </p>
      {post.hasImage && (
      ...
      <div
        style={{ display: 'flex', gap: 24, paddingTop: 12, borderTop: '1px solid var(--border)' }}
      >
        <button
          className={`sf-action${liked ? ' liked' : ''}`}
          onClick={() => setLiked(!liked)}
          aria-label="Like post"
        >
          {liked ? '♥' : '♡'} {post.likes + (liked ? 1 : 0)}
        </button>
        <button className="sf-action" aria-label="View comments">
          💬 {post.replies}
        </button>
        <button className="sf-action" aria-label="Share post">
          ↗ Share
        </button>
      </div>
      {post.id === '1' && (
        <div style={{ marginTop: 12, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              {
                author: 'Julia Park',
                text: 'This is gorgeous. The spacing alone tells me how much thought went into it.',
                time: '1h ago',
              },
              {
                author: 'Ana Souza',
                text: 'The type hierarchy is perfect. Instrument Serif was the right call.',
                time: '45m ago',
              },
            ].map((c, i) => (
  ...
  return (
    <div className="sf-root" style={{ position: 'relative' }}>
      ...
      <section style={{ padding: '64px 32px 100px' }}>
        <div
          className="sf-feed-grid reveal reveal-d3"
          style={{
            maxWidth: 1000,
            margin: '0 auto',
            display: 'grid',
